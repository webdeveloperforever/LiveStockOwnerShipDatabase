...
<head>
	...
	<script type="text/javascript">
		//var baseUrl="delete_petdetils.php?id=" . $row['pet_id'] ;
		function ConfirmDelete(url) {
			if (confirm("Are you Sure to Delete Account?")) {
				location.href = 'delete_petdetils.php?id=' + url;
			}
		}
	</script>
</head>
...
<body>
	...
	<h1>View All Live Stock Details</h1>
	<form method="post">
	...
	<table>
		<tr>
			...
			<th>LiveStockKeywords</th>
			<th>LiveStockFeature1</th>
			<th>LiveStockFeature2</th>
			<th>LiveStockImages</th>
		...
		if (isset($_POST['search_btn'])) {
			...
			$query = $con->prepare("select * from petdetails where pet_name like '%$getdata%' or pet_type like '%$getdata%'");
		...
		while ($row = $query->fetch()) :
			...
			<td><a type='button' onClick='ConfirmDelete(" . $row['pet_id'] . ")' href='###'>Delete</a></td>
